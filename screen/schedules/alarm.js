...
import {View, Platform, Text, ScrollView, StyleSheet, TouchableOpacity} from 'react-native';
...
export default function Alarm(){
  ...
  const onChange = (event, selectedDate) => {
    console.log(moment(selectedDate).format('LT'));
    ...
    <ScrollView >
        {listScedule.map((e,index) => {
          return (
            ...
            <View>
                <Text style={{ fontSize: 30 }}>{moment(e.date).format('LTS')}</Text>

            </View>
            <View style={{width: 200,flexDirection: 'row', justifyContent: 'flex-end',alignItems: 'center'}}>
                <Text style={styles.time}>{moment(e.date).format('DD/MM')}</Text>
                <TouchableOpacity onPress={() => cancleOne(e.id)}>
                  <Icon name="power-off" size={30} style={{paddingHorizontal: 20}}/>
                </TouchableOpacity>
            </View>
            </View>
          );
