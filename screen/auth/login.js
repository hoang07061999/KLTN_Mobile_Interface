...
class Login extends Component {
  ...
  componentDidMount() {
    GoogleSignin.configure({
      webClientId:
        '563776868206-q6214nihon0mqsrkqe3sbsruvirshjcn.apps.googleusercontent.com',
      offlineAccess: true,
    });
    this.isSignedIn();
  }
    ...
    return (
      <View style={styles.container}>
        <Image
          ...
          source={require('../../assets/image/logo/logo_gra.png')}
        />
        <View
          style={{
            width: '100%',
          ...
          <View style={styles.wrapperInput}>
            <TextInput
              value={this.state._txtEmail}
              style={styles.email}
              placeholder="Email"
              onChangeText={(text) => this._onChange(text, '_txtEmail')}
            />
            <TextInput
              value={this.state._txtPass}
              secureTextEntry
              style={styles.pass}
              placeholder="Mật khẩu"
              onChangeText={(text) => this._onChange(text, '_txtPass')}
            />
          </View>
          {validateEmail ? (
            <Text style={{paddingBottom: 15, color: 'red'}}>
          ...
          <TouchableOpacity
            disabled={
              (validateEmail === false && validatePass === false) ||
              (_txtEmail !== '' && validatePass === false)
                ? false
                : true
            }
            style={[
          ...
          <Text
            style={{paddingTop: 10, textDecorationLine: 'underline'}}
        ...
        <TouchableOpacity
          style={{marginTop: 30}}
          onPress={() => this.props.navigation.navigate('searchEmail')}>
